...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_CreateConto` (IN `NomeConto` VARCHAR(255), IN `Saldo` DECIMAL(10,2), IN `IdRisparmio` INT, IN `IDDebit` INT, IN `IDCredit` INT, IN `IDObiettivo` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_CreateProfilo` (IN `NomeProfilo` VARCHAR(255), IN `Saldo_totale` DECIMAL(10,2), IN `Email` VARCHAR(255), IN `Password` VARCHAR(255))   BEGIN
    INSERT INTO Profili (NomeProfilo, Saldo_totale, Email, Password)
    VALUES (NomeProfilo, Saldo_totale, Email, Password);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_CreateTransazione` (IN `Entrata_Uscita` BOOLEAN, IN `Importo` DECIMAL(10,2), IN `IDTemplate` INT, IN `IDConto` INT, IN `DataTransazione` DATE, IN `IDCategoriaPrimaria` INT, IN `IDCategoriaSecondaria` INT, IN `Descrizione` VARCHAR(255))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_DeleteConto` (IN `IDConto` INT)   BEGIN
    DELETE FROM Conto WHERE IDConto = IDConto;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_DeleteProfilo` (IN `IDProfilo` INT)   BEGIN
    DELETE FROM Profili WHERE IDProfilo = IDProfilo;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_DeleteTransazione` (IN `ID` INT)   BEGIN
    DELETE FROM Transazione WHERE ID = ID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_UpdateConto` (IN `IDConto` INT, IN `NomeConto` VARCHAR(255), IN `Saldo` DECIMAL(10,2), IN `IdRisparmio` INT, IN `IDDebit` INT, IN `IDCredit` INT, IN `IDObiettivo` INT)   BEGIN
    UPDATE Conto
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_UpdateProfilo` (IN `IDProfilo` INT, IN `NomeProfilo` VARCHAR(255), IN `Saldo_totale` DECIMAL(10,2), IN `Email` VARCHAR(255), IN `Password` VARCHAR(255))   BEGIN
    UPDATE Profili
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_UpdateTransazione` (IN `ID` INT, IN `Entrata_Uscita` BOOLEAN, IN `Importo` DECIMAL(10,2), IN `IDTemplate` INT, IN `IDConto` INT, IN `DataTransazione` DATE, IN `IDCategoriaPrimaria` INT, IN `IDCategoriaSecondaria` INT, IN `Descrizione` VARCHAR(255))   BEGIN
    UPDATE Transazione
...
ALTER TABLE conto
  ...
  ADD KEY IdRisparmio (IdRisparmio),
  ADD KEY IDDebit (IDDebit),
  ADD KEY IDCredit (IDCredit),
  ADD KEY IDObiettivo (IDObiettivo);
...
ALTER TABLE assconti
  ADD CONSTRAINT assconti_ibfk_1 FOREIGN KEY (IDProfilo) REFERENCES profili (IDProfilo),
  ADD CONSTRAINT assconti_ibfk_2 FOREIGN KEY (IDConto) REFERENCES conto (IDConto);

ALTER TABLE categoriaprimaria
  ADD CONSTRAINT categoriaprimaria_ibfk_1 FOREIGN KEY (IDBudget) REFERENCES budgetmax (IDBudget);

ALTER TABLE categoriasecondaria
  ADD CONSTRAINT categoriasecondaria_ibfk_1 FOREIGN KEY (IDCategoriaPrimaria) REFERENCES categoriaprimaria (ID);

ALTER TABLE conto
  ADD CONSTRAINT conto_ibfk_1 FOREIGN KEY (IdRisparmio) REFERENCES risparmi (ID),
  ADD CONSTRAINT conto_ibfk_2 FOREIGN KEY (IDDebit) REFERENCES debit (ID),
  ADD CONSTRAINT conto_ibfk_3 FOREIGN KEY (IDCredit) REFERENCES credit (ID),
  ADD CONSTRAINT conto_ibfk_4 FOREIGN KEY (IDObiettivo) REFERENCES obiettivifinanziari (ID);

ALTER TABLE template_transazioni
  ADD CONSTRAINT template_transazioni_ibfk_1 FOREIGN KEY (IDConto) REFERENCES conto (IDConto),
  ADD CONSTRAINT template_transazioni_ibfk_2 FOREIGN KEY (IDCategoriaPrimaria) REFERENCES categoriaprimaria (ID),
  ADD CONSTRAINT template_transazioni_ibfk_3 FOREIGN KEY (IDCategoriaSecondaria) REFERENCES categoriasecondaria (ID);

ALTER TABLE transazione
  ADD CONSTRAINT transazione_ibfk_1 FOREIGN KEY (IDTemplate) REFERENCES template_transazioni (IDTemplate),
  ADD CONSTRAINT transazione_ibfk_2 FOREIGN KEY (IDConto) REFERENCES conto (IDConto),
  ADD CONSTRAINT transazione_ibfk_3 FOREIGN KEY (IDCategoriaPrimaria) REFERENCES categoriaprimaria (ID),
  ADD CONSTRAINT transazione_ibfk_4 FOREIGN KEY (IDCategoriaSecondaria) REFERENCES categoriasecondaria (ID);
